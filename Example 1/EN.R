...
ga1 <- 0.95
theta <- 0.05
alpha.v=c(exp(seq(log(0.1),log(0.5),length.out= 15))) ##No.10 is best
lg=length(alpha.v)
...
for (M in 1:100) {
  ...
  for (j in 1:lg) {
    pen=c(rep(1,2*p))
    lmnew=lm1
    ANS1=newSVMADMMEnet(Y,X,c(alpha.v[j]*lmnew*pen[1:p],(1-alpha.v[j])*lmnew*pen[p+1:m]),
                          G,n,p,ItM=3*10^5,eabsn=10^(-6),erel=10^(-5))
    bet1=ANS1[[1]][-1]
    b0=ANS1[[1]][1]
    cvnew=targetf(Yva,Xva,bet1,(1-alpha.v[j])*lmnew*diag(p),c(alpha.v[j]*lmnew*pen[1:p],pen[p+1:m]),b0)
